...
public class pMatrix {
    ...
    /**
     * turn matrix into array, left to right.
     */
    ...
    /**
     * calculate stats for this matrix.
     */
    public double[] stats(){
        double sum = 0, sum2=0;
        for(int i=0;i<rows;i++){
        ...
        return new double[] { sum, avg, sum2, variance, stddev };
    }
    ...
    /**
     * variance
     * second moment - first moment squared/n
     */
                ...
                for(int i=0;i<rgb[0].rows;i++){
                    for(int j=0;j<rgb[0].cols;j++){
                        pixels[(i<<rgb[0].pitch)+j] = 0xFF << 24 | 
                            ((int)(rgb[0].arr[(i<<rgb[0].pitch)+j]) % 0x100) << 16 | 
                            ((int)(rgb[1].arr[(i<<rgb[1].pitch)+j]) % 0x100) << 8 | 
